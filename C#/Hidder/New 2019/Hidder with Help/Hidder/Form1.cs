...
namespace Hidder
{
    public partial class Form1 : Form
    {
        ...
        private void buttonProcessHide_Click(object sender, EventArgs e)
        {
            ...
            {
                if (File.Exists(textBoxMainPath.Text) && ((radioButtonHideFolder.Checked && Directory.Exists(textBoxHideFiles.Text)) || ((!radioButtonHideFolder.Checked && File.Exists(textBoxHideFiles.Text)))))
                {
                ...
                else MessageBox.Show("Необходимо указать существующие пути!");
            }
        ...
        private void buttonExtract_Click(object sender, EventArgs e)
        {
            ...
            else MessageBox.Show("Необходимо указать путь к существующему файлу");
        }
        ...
        private void threadGetInfoExtract(Hidder hidder)
        {
            ...
            if (hidder.getCountHidedFiles() > 0 || hidder.getCountHidedFolders() > 0)
            {
                ...
                if (hidder.getExtractedFiles() == 0 && hidder.getExtractedFolders() == 0)
                {
                    ...
                    else statusMsg = "не получается распаковать";
                }
            }
            else
            {
        ...
        {
            //Help.ShowHelp(this, "Resources\\help",HelpNavigator.TableOfContents);
            //helpProvider1.GetShowHelp();
            using (FileStream fileStream = new FileStream("help.chm", FileMode.Create, FileAccess.Write))
            {
