...
const Progrm = 'BotAct';
      ...
      PanelColor = clGrayText;  ColorActNext = clGreen;   ColorActDo = clMaroon;
      ...
      3: begin
          ...
          begin
            for j := RptBeg to RptEnd do
              if j <> PanelN then begin Panel[j].Color := ColorActNext; if (j <> RptBeg) then Sleep(autoSleep);
               Panel[j].Color := ColorActDo; DoAct(j); Panel[j].Color := PanelColor; end;
...
//  sleep(firstSleep);
  ...
  begin
    ...
    Panel[i].Color := ColorActNext;
    if status then begin Sleep(autoSleep); Panel[i].Color := ColorActDo; Self.DoAct(i) end else break;
...
begin
  ...
  case Event of
    ...
    begin
      ...
      begin
        ...
        if Event = 0 then
         ...
         Edit[2][ActNum].TabStop := True;
         Edit[4][ActNum].TabStop := True;
        end;

      if Event = 0 then
      begin
            Edit[2][ActNum].TabStop := False;
            Edit[4][ActNum].TabStop := False;
      end else if Event = 1 then
      begin
            Edit[2][ActNum].TabStop := True;
            Edit[4][ActNum].TabStop := True;
      end;
      ...
      begin
        ...
        Edit[3][ActNum].Name := 'EditT'+inttostr(ActNum);
      ...
      //Sleep
    2: begin
      if Edit[3][ActNum] = nil then begin CreateEdit(Edit[3][ActNum],ActNum); Edit[3][ActNum].Name := 'EditT'+inttostr(ActNum); end; //MlSec
    ...
    3: begin
      ...
      if Edit[3][ActNum] = nil then begin CreateEdit(Edit[3][ActNum],ActNum); Edit[3][ActNum].Name := 'EditT'+inttostr(ActNum); end;
      ...
      Edit[2][ActNum].TabOrder := 1;
      Edit[3][ActNum].TabOrder := 2;
      Edit[4][ActNum].TabOrder := 3;

      Edit[2][ActNum].TabStop := True;
      Edit[3][ActNum].TabStop := True;
      Edit[4][ActNum].TabStop := True;
