...
char userBashHistory[255] = "\0";
char userTmpDir[255] = "\0";
//char userLeftCommandsPath[255] = "\0";
char userExpFile[] = "exp";
char userTimeFile[] = "time";
char userLeftCommandsFile[] = "leftC";
...
//  char template[] = "/tmp/rpg.XXXXXXXX";
  char template[255] = "/tmp/rpg-\0";
  strcat(template,getlogin());
...
int createTmpDir(){
  struct stat st = {0};
  char path[255] = "/tmp/rpg-";
  strcat(path,getlogin());
  strcpy(userTmpDir,path);
  if (stat(path, &st) == -1){
	  char cmd[255] = "mkdir ";
	  strcat(cmd,path);
	  system(cmd);
...
int inFileLines(const char *pth)
{
  FILE *f=fopen(pth,"r");
  int lines = 1;
  char c;
...
//    system("clear");
  ...
  } else if (strCompare(tmpArg,"background")==1)
  {
	...
	char lastHistory[255],nowHistory[255],difference[255];
	strcpy(lastHistory,userTmpDir);
	strcat(lastHistory,"/HistoryLast");
	strcpy(nowHistory,userTmpDir);
	strcat(nowHistory,"/HistoryNow");
	strcpy(difference,userTmpDir);
	strcat(difference,"/HistoryDiffs");
	char cmdLast[555],cmdNow[555],cmdDiff[555];
	char *homedir = getenv("HOME");
	char bashHistory[555];
	strcpy(bashHistory,homedir);
	strcat(bashHistory,"/.bash_history");

	strcpy(cmdNow,"cp ");
	strcat(cmdNow,bashHistory);
	strcat(cmdNow," ");
	strcat(cmdNow,nowHistory);

	strcpy(cmdLast,"cp ");
	strcat(cmdLast,nowHistory);
	strcat(cmdLast," ");
	strcat(cmdLast,lastHistory);

	strcpy(cmdDiff,"diff ");
	strcat(cmdDiff,nowHistory);
	strcat(cmdDiff," ");
	strcat(cmdDiff,lastHistory);
	strcat(cmdDiff," -n > ");
	strcat(cmdDiff,difference);

//	printf("\n%s\n%s\n%s\n",cmdNow,cmdLast,cmdDiff);
	system(cmdNow);
	int nowHistoryCommands = inFileLines(nowHistory);
	int lastHistoryCommands = 0;

//	printf("\n nowComms = %d      lastComms = %d",nowHistoryCommands,lastHistoryCommands);
	//printf("\n\n %s \n\n",lastHistory);

	if (fileExists(lastHistory)==0)
	{
	  system(cmdLast);
	  system(cmdDiff);
	} else
	{
	  lastHistoryCommands = inFileLines(lastHistory);
	  system(cmdDiff);
	  system(cmdLast);
	  char tmp[255] = "\0";
	  FILE *f = fopen(difference,"r");
	  ...
	  rewind(f);
	  int diffs = 0;
	  fscanf(f,"%s%d",&tmp,&diffs);
	  fclose(f);
	  int Lefted = 0;

	  char userLeftCommandsPath[255] = "\0";
	  strcpy(userLeftCommandsPath,userDir);
	  strcat(userLeftCommandsPath,"/");
	  strcat(userLeftCommandsPath,userLeftCommandsFile);

//	  printf("\n%s\n",userLeftCommandsPath);
	  if(fileExists(userLeftCommandsPath)==0)
	  {
		FILE *f = fopen(userLeftCommandsPath,"w");
		fprintf(f,"%d",diffs % ExpPerCommands);
		fclose(f);
	  } else
	  {
		FILE *f = fopen(userLeftCommandsPath,"r");
		fscanf(f,"%d",&Lefted);
		fclose(f);
		f = fopen(userLeftCommandsPath,"w");
		fprintf(f,"%d",(diffs+Lefted) % ExpPerCommands);
		fclose(f);
	  }
	  int expAdd = (int)(diffs+Lefted)/ExpPerCommands;
	  if(expAdd > 0)
	  {
...
/*  while(1==1)
  {
    system("echo '!!' > /tmp/rpg-KirKhal/test");
    sleep(10);
  }*/
